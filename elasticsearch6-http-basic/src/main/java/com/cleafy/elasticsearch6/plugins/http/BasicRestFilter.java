...
//        ipwhitelist.add("127.0.0.1");
        this.httpBasicAuthenticator = new HttpBasicAuthenticator(settings, 
        		new AuthCredentials(settings.get(Globals.SETTINGS_USERNAME), settings.get(Globals.SETTINGS_PASSWORD)));
    ...
    private boolean checkAndAuthenticateRequest(RestRequest request, RestChannel channel, NodeClient client) throws Exception {
        ElasticsearchException forbiddenException = new TransportException("Unauthorized");
        ...
        try {
            ...
            RestResponse rr = new BytesRestResponse(channel, RestStatus.UNAUTHORIZED, forbiddenException);
            rr.addHeader("WWW-Authenticate", "Basic realm=\"ElasticSearch Server\"");
            channel.sendResponse(rr);
        } catch (Exception e) {
            if (this.isUnauthLogEnabled) { LoggerUtils.logUnAuthorizedRequest(request, getClass()); }
            RestResponse rr = new BytesRestResponse(channel, RestStatus.UNAUTHORIZED, forbiddenException);
            rr.addHeader("WWW-Authenticate", "Basic realm=\"ElasticSearch Server\"");
